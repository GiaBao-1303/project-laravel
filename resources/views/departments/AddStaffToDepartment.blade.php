...
<body>
    <form method="POST" action="/departments/{{ $department->DepartmentID }}/assignment" class="container-xl">
        @csrf
        <div class="table-responsive">
            <div class="table-wrapper">
                <div class="table-title">
                    <div class="row">
                        <div class="col-sm-5">
                            <h2>Thêm người dùng vào phong ban: <b>{{ $department->Name }}</b></h2>
                        </div>
                        <div class="col-sm-7">
                    ...
                    <tbody>
                        @foreach ($emloyees as $emloyee)
                            <tr>
                                <td>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox"
                                            name="business_entity_ids[employee_{{ $emloyee->BusinessEntityID }}][BusinessEntityID]"
                                ...
                                <td>
                                    <select id="ShiftID_{{ $emloyee->BusinessEntityID }}"
                                        name="business_entity_ids[employee_{{ $emloyee->BusinessEntityID }}][ShiftID]"
                                        class="form-select">
                                        <option value=""
                                            {{ old('ShiftID_' . $emloyee->BusinessEntityID) ? '' : 'selected' }}>Lựa
                                            Chọn Ca Làm</option>
                                        @foreach ($shifts as $shift)
                                            <option value="{{ $shift->ShiftID }}"
                                                {{ old('ShiftID_' . $emloyee->BusinessEntityID) === (string) $shift->ShiftID ? 'selected' : '' }}>
                                                {{ $shift->Name }}
                                            </option>
                                        @endforeach
                                    </select>
